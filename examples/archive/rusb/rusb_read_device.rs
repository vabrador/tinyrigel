...
fn open_device<T: UsbContext>(
  ...
  for device in devices.iter() {
    ...
    println!("Checking device {:04x}:{:04x}", device_desc.vendor_id(), device_desc.product_id());
    ...
    if device_desc.vendor_id() == vid && device_desc.product_id() == pid {
      ...
      match device.open() {
        ...
        Err(e) => {
          println!("Matched a device {:04x}:{:04x}, but it failed to open. Device error was: {}", vid, pid, e);
          continue
        },
      }
    }

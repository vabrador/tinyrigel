...
mod tests_windows_winrt {
    ...
    fn can_retrieve_rigel_frame() -> windows::Result<()> {
        ...
        // MediaFrameSource attempt //
        // ---
        // This part doesn't work because the rigel_cap was not constructed with frame sources.
        ...
        //
        // We'll pass the transmitter to the frame callback thead and use the receiver to know when the frame callback thread has finished writing the test frame to disk.
        // (Unfortunately the write call is not guaranteed to have finished the disk write operation... So we spin a bit afterwards for safety)
        let (tx, rx) = sync_channel::<u32>(0);

        let evt_token = rigel_media_frame_reader.frame_arrived(TypedEventHandler::new(move |reader: &Option<MediaFrameReader>, args: &Option<MediaFrameArrivedEventArgs>| {
            let reader = reader.as_ref().unwrap();
            let _args = args.as_ref().unwrap();

            let latest_frame = reader.try_acquire_latest_frame()?;
            ...
            // let mut frame_data_vec: Vec<u8> = Vec::with_capacity(FRAME_NUM_BYTES_YUY2);
            // let mut frame_data_arr: [u8; FRAME_NUM_BYTES_YUY2] = [0; FRAME_NUM_BYTES_YUY2];
            ...
            // Ultimately we probably don't want this? And we want to write to a buffer..?
            match tx.send(0) {
        ...
        // Receive a single signal from a frame thread...
        let _sig_int = rx.recv().unwrap();
        ...
        // Wait a second for the pending file write to not be interrupted by the test thread ending...
